...
if __name__ == '__main__':
    ...
    if os.path.exists(conf.total_user_recall_path):
        total_user_recall_df = pd.read_csv(
            conf.total_user_recall_path,
            dtype={'user_id': np.int, 'item_id': np.int}
        )

        total_user_recall_df.loc[:, 'user_id'] = total_user_recall_df['user_id'].astype(np.str)
        total_user_recall_df.loc[:, 'item_id'] = total_user_recall_df['item_id'].astype(np.str)
    else:
        raise Exception('no cached total user recall result!')
